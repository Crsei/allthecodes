...
use allthecodes_engine::types::config::QueryParams;
use allthecodes_engine::types::message::QueryYield;
...
use allthecodes_engine::types::state::{BudgetTracker, TokenBudgetDecision};
use allthecodes_engine::types::transitions::Continue;

use allthecodes_services::tool_use_summary::{self, ToolInfo};
    ...
    stream! {
        ...
        'query_loop: loop {
            ...
            for agent in &completed_agents {
                ...
                let sys_msg = Message::System(allthecodes_engine::types::message::SystemMessage {
                    uuid: Uuid::new_v4(),
                    timestamp: chrono::Utc::now().timestamp_millis(),
                    subtype: allthecodes_engine::types::message::SystemSubtype::Informational {
                        level: allthecodes_engine::types::message::InfoLevel::Info,
                    },
                    content,
                });
            ...
            let generation_input = allthecodes_services::langfuse::convert::convert_generation_input(
            ...
            let (assistant_message, streaming_tool_executor) = loop {
                ...
                let mut generation_span = deps.langfuse_trace().as_ref().and_then(|trace| {
                    allthecodes_services::langfuse::create_generation_span(
                        trace,
                        &attempt_model,
                ...
                let mut event_stream = match stream_result {
                    ...
                    Err(e) => {
                        ...
                        allthecodes_services::langfuse::finish_generation_span(
                ...
                allthecodes_services::langfuse::finish_generation_span(
                    generation_span.take(),
                    Some(allthecodes_services::langfuse::convert::convert_assistant_output(
                        &assistant_message,
                    )),
