...
use allthecodes_api::api::streaming::CompletedToolUse;
use allthecodes_engine::types::message::{
    AssistantMessage, ContentBlock, Message, MessageContent, StreamEvent, ToolResultContent,
    UserMessage,
};
use allthecodes_engine::types::state::QueryLoopState;
use allthecodes_engine::types::tool::{ToolProgress, Tools};
...
pub(crate) async fn execute_tool_calls(
    ...
    tools: &allthecodes_engine::types::tool::Tools,
    parent_message: &AssistantMessage,
    on_progress: Option<Arc<dyn Fn(ToolProgress) + Send + Sync>>,
) -> Vec<ToolExecResult> {
    let mut results = Vec::new();
    ...
    for (batch_index, (is_concurrent, batch)) in batches.into_iter().enumerate() {
        ...
        allthecodes_services::langfuse::end_span(batch_span);
    }
...
mod tests {
    ...
    use allthecodes_engine::types::message::{StreamEvent, Usage};
    use allthecodes_engine::types::state::AutoCompactTracking;
    use allthecodes_engine::types::tool::{Tool, ToolResult, ToolUseContext, Tools};
    ...
    fn tool_result_user_message_preserves_model_content_and_preview() {
        ...
        let image = ContentBlock::Image {
            source: allthecodes_engine::types::message::ImageSource {
