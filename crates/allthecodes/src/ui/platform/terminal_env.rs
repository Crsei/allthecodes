...
//! |                             | that behave badly.                                 |
//! | `ALLTHECODES_ENABLE_MOUSE_CAPTURE` | `0` turns off TUI mouse capture so native |
...
pub struct TerminalEnvConfig {
    ...
    /// `ALLTHECODES_DISABLE_MOUSE=1` or `ALLTHECODES_ENABLE_MOUSE_CAPTURE=0`.
...
mod tests {
    ...
    fn no_flicker_0_disables_sync_updates() {
        let cfg = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_NO_FLICKER", "0")]);
        assert!(!cfg.sync_updates);
    }

    #[test]
    fn no_flicker_1_keeps_sync_updates_on() {
        let cfg = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_NO_FLICKER", "1")]);
        assert!(cfg.sync_updates);
    }
    ...
    fn disable_mouse_accepts_common_truthy() {
        for value in ["1", "true", "YES", "on"] {
            let cfg = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_DISABLE_MOUSE", value)]);
            assert!(
                cfg.disable_mouse,
    ...
    fn enable_mouse_capture_opts_into_mouse_events() {
        for value in ["1", "true", "YES", "on"] {
            let cfg =
                TerminalEnvConfig::from_iter(vec![("ALLTHECODES_ENABLE_MOUSE_CAPTURE", value)]);
            assert!(
                !cfg.disable_mouse,
    ...
    fn legacy_disable_mouse_false_still_enables_mouse_capture() {
        let cfg = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_DISABLE_MOUSE", "0")]);
        assert!(!cfg.disable_mouse);
    }
    ...
    fn scroll_speed_parses_and_clamps() {
        let cfg = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_SCROLL_SPEED", "12")]);
        assert_eq!(cfg.scroll_speed, 12);

        let cfg_hi = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_SCROLL_SPEED", "9999")]);
        assert_eq!(cfg_hi.scroll_speed, TerminalEnvConfig::MAX_SCROLL_SPEED);

        let cfg_zero = TerminalEnvConfig::from_iter(vec![("ALLTHECODES_SCROLL_SPEED", "0")]);
