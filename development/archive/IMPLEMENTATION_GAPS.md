...
- [archive/COMPLETED_FULL.md](COMPLETED_FULL.md)
...
| Agentic workflow injection defense | task4–6 第一版代码已落地，workspace clippy gate 待完成 | setup-chain / supply-chain 有界静态扫描、scanner Deny/Ask 接入 canonical tool gate、精确一次性审批、redacted security record/audit/IPC/UI metadata 已实现。离线 fixtures 验证 scanner/policy 契约；engine lifecycle 测试独立验证真实执行边界的零工具调用、decision 落盘和输入级一次性审批；audit-export 测试验证 security decision 的顺序 hash chain。复杂 YAML/脚本语义仍可能产生保守 Ask。2026-07-13 focused tests、fmt、diff check 和 workspace release build 均通过；workspace all-targets clippy 仍被共享工作区 `allthecodes-mcp/src/runtime.rs:124` 缺失 `take_installed_manager` 的测试编译错误阻塞。 |
| Runtime verification evidence / Session Report | 第一版闭环及 review remediation 已落地，完整 traceable logging 仍未完成 | canonical verification/artifact/report records、fail-closed 单命令证据分类、未知 policy 拒绝、`Incomplete` SDK/Agent 终态传播、最多三轮 verify-continue、canonical changed-files + terminal usage/cost 投影、redacted/tamper-linked `SessionReportV1`、Web/IPC/TUI/telemetry surfaces 和 deterministic e2e 已实现。仍缺统一 durable runtime audit event log、完整 submit/turn/request/event correlation、daemon/IPC/permission/stream/progress 覆盖，以及 audit export 以该事实源为主。workspace release build 已通过；workspace clippy 仍受共享工作区缺失的 `allthecodes-mcp::take_installed_manager` 测试辅助函数阻塞。 |
...
- [plan/cfg-test-production-wiring-plan-2026-05-21.md](plan/cfg-test-production-wiring-plan-2026-05-21.md): Phase 1-16 已完成；保留为实现记录，后续可迁移到 archive，不再作为活跃 TODO。
...
- Command-surface production wiring: 2026-05-21 cfg-test wiring pass completed `/agents` create/edit, `/mcp` detail/tools, `/permissions`, `/tasks`, `/team`, dialog helpers, tabs helpers, command palette/runtime snapshots, and production no-arg routing. This is no longer a tracked gap.
- Persistent history: Ctrl+R uses workspace persistent history where backend data is available; cross-session history quality still depends on durable reader coverage and should remain under UI/runtime residual tracking.
