...
- `development/archive/plan/daemon-usability-plan.md` records that Phase 5 only landed the local control-token security boundary. It explicitly leaves bridge worker network registration and remote submit/abort/permission mapping for later.
- `development/reference/DAEMON_OPERATIONS.md` says bridge worker registration, remote submit/abort/permission mapping, and result return are still incomplete.
...
- Existing cc-rust daemon plan: `development/archive/plan/daemon-usability-plan.md`
- Existing daemon operations doc: `development/reference/DAEMON_OPERATIONS.md`
...
- Update `development/archive/IMPLEMENTATION_GAPS.md`:
...
- Update `development/archive/KNOWN_ISSUES.md` only if a user-visible limitation remains.
