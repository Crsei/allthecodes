...
目标: 将 [`development/archive/RATATUI_UI_PARITY.md`](../RATATUI_UI_PARITY.md) 中标记为
`⚠️ 部分` 和 `❌ 缺失` 的 ratatui UI 功能收敛成正式实现计划，尤其补上目前没有
进入 `development/archive/plan/` 的功能跟踪。本文档不实现代码，只把缺口、归属、顺序、验收和
验证路径固定下来。
...
1. `development/archive/RATATUI_UI_PARITY.md`：状态从 `❌`/`⚠️` 改为 `✅` 或明确 crop/deferred。
2. `docs/WORK_STATUS.md`：当前 release gate 更新。
3. `development/archive/IMPLEMENTATION_GAPS.md`：只保留仍未完成的全量构建 TODO。
4. `development/archive/COMPLETED_FULL.md`：记录完成项和验证命令。
