...
| PROVIDER-002 | 高 | Fixed | OpenAI-compatible SSE / CLI tool sessions | 原始 60/120 秒长流、EOF 伪完成与 stream reconnect 修复保持有效；默认配置约 218 秒的 `gpt-5.6-sol` 调用成功。后续暴露的 post-tool 刷新、orphan resume/HTTP 400 伪成功、跨 turn Read/Edit 状态与无界 validation loop 也已收口。 | 已落地非阻塞工具刷新、rollout-first tool result 耐久化、resume 三路协议修复、session-owned file cache、tool-error loop guard 与统一非交互失败退出。真实 OAuth 受控恢复无 400 且未重放 4 个 Write；最终 SVG favicon 验收严格 Read=1、Edit=1、Write=0；Playwright 的 HTTP/桌面/移动/键盘/reduced-motion 与错误计数矩阵通过。证据见唯一权威计划及 `development/worktree-workflow-artifacts/2026-07-20-codex-cli-session-tool-state-hardening.html`。 |

当前没有开放 provider 项。其他已关闭记录见
