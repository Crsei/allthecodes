...
| [pty-tui-e2e-timing-plan.md](pty-tui-e2e-timing-plan.md) | `development/test/pty-tui-e2e-timing-plan.md`（本目录内） | **Phase 1+2 已实现**（信号化等待 + cleanup 节流 + 串行锁 lift；2026-07-18 followup 修复 nextest override 滤片 `binary(pty_tui_e2e)` 让 `max-threads=1` 真正生效，全套件离线 217 测试 nextest 回归转绿）；nextest `max-threads` 待 Phase 3 per-session workspace 隔离后再升 2 → 4） | ✅ 部分 |
