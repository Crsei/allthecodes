...
| 阶段 2 | Processor 模式 | ~25% | ⚠️ 基础设施完备，仅迁移了 1/7 |
...
其他 6 个 Processor 组 | ❌ **未迁移** — Session、Agent、File、Skill/Plugin、Chat、Health 均为旧自由函数

**迁移优先级与实际进度对比**:

| 计划优先级 | Processor | 实际状态 |
|------------|-----------|----------|
| 1 | HealthProcessor | ❌ 未开始 |
| 2 | CapabilitiesProcessor | ✅ 已完成 |
| 3 | SessionProcessor | ❌ 未开始 |
...
**问题**: 
- 大部分 handlers 未从 Processor 模式受益（标准错误路径、序列化集成）
...
```
**原因**: `normalized_json_object` 的签名为 `fn(Option<Value>) -> Option<Value>`，但两处调用 `.and_then(normalized_json_object)`（第 516 行和第 704 行）需要 `fn(Value) -> Option<Value>`。

**修复**: 将签名改为 `fn normalized_json_object(value: Value) -> Option<Value>`。
...
cargo check -p allthecodes-web       — 因 providers.rs 编译错误失败
...
1. **🔴 高 — 修复 providers.rs 编译错误** — 阻止整个 web crate 编译
2. **🟡 中 — 统一 ApiError** — 消除双重 ApiError 类型，迁移 `api_fallback_handler`
3. **🟡 中 — 继续 Processor 迁移** — 优先迁移 SessionProcessor（核心域，为 processor 模式提供负载验证）
